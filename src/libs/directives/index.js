...
const wheelFn = (e, el, binding, vnode, oldNode) => {
  e && e.preventDefault()
  let wheelDelta = e ? e.wheelDelta || -e.detail * 24 : 1
  if (wheelDelta) {
    ...
    if (e) {
      if (wheelDelta > 0) {
        if (scale >= 2.5) return
        scale = (Number(scale) + unit).toFixed(2)
      } else {
        if (scale <= 0.1) return
        scale = (Number(scale) - unit).toFixed(2)
      }
...
export default {
  flowWheel: {
    bind: (el, binding, vnode, oldNode) => {
      wheelFn(null, el, binding, vnode, oldNode)
      if (!binding) {
        return
      }
      el.onmousewheel = e => {
    ...
    update: (el, binding, vnode, oldNode) => {
      ...
      efContainer.style.transition = 'transform .4s ease-out';
